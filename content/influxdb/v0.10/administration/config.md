...
See the [installation documentation](../../introduction/installation/#generate-a-configuration-file) for more detail on generating and using configuration files.
...
number of [databases](../../concepts/glossary/#database),
[measurements](../../concepts/glossary/#measurement), and
unique [series](../../concepts/glossary/#series).  Setting
...
For step-by-step instructions on setting up an InfluxDB cluster, see [Cluster Setup](../../guides/clustering/).
...
Retention policy auto-creation automatically creates a [`default` retention policy](../../concepts/glossary/#retention-policy-rp) when a database is created.
...
If you have performance concerns, you will also want to make this setting different from the dir in the [[data]](#data) section.
...
If the write times out, it may still succeed on the remote node but the client node stops waiting and queues it in [hinted handoff](../../concepts/glossary/#hinted-handoff).
...
If the write times out, it may still succeed but we stop waiting and queue those writes in [hinted handoff](../../concepts/glossary/#hinted-handoff).
...
If you want to use a retention policy other than the seven-day retention policy, you must [create](../../administration/administration/#retention-policy-management) it.
...
>**Note:** HTTPS must be enable for the [[http]](#http) service for the admin UI to function properly using HTTPS.
...
Note that the InfluxDB [command line interface (CLI)](../../tools/shell/) connects to the database using the HTTP API.
...
For more information, see the Query String Parameters for Writes section in the [Line Protocol Syntax Reference ](../../write_protocols/write_syntax/).
...
See the [UDP page](../../write_protocols/udp/) for more information.
...
This section controls how [continuous queries (CQs)](../../concepts/glossary/#continuous-query-cq) run within InfluxDB.
