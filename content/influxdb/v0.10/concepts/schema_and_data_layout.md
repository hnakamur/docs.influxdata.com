...
Every InfluxDB use case is special and your [schema](../glossary/#schema) will reflect that uniqueness.
...
* *Encode meta data in tags*
    ...
    [Tags](../glossary/#tag) are indexed and [fields](../glossary/#field) are not indexed.
    ...
    In general, your queries should guide what gets stored as a tag and what gets stored as a field:
      ...
      * Store data in fields if you plan to use them with an [InfluxQL function](../../query_language/functions/)
      * Store data in fields if you *need* them to be something other than a string - [tag values](../glossary/#tag-value) are always interpreted as strings
...
* *Avoid using InfluxQL Keywords as identifier names*
    ...
    Identifiers are are database names, [retention policy](../glossary/#retention-policy-rp) names, [user](../glossary/#user) names, [measurement](../glossary/#measurement) names, [tag keys](../glossary/#tag-key), and [field keys](../glossary/#field-key).
...
* *Don't have too many series*
    ...
    See [Hardware Sizing Guidelines](../../guides/hardware_sizing/#general-hardware-guidelines-for-a-single-node) for [series cardinality](../glossary/#series-cardinality) recommendations based on your hardware.

    [Tags](../glossary/#tag) that specify highly variable information like UUIDs, hashes, and random strings can increase your series cardinality to uncomfortable levels.
...
* *Don't differentiate data with measurement names*
    ...
    InfluxDB queries merge data that fall within the same [measurement](../glossary/#measurement); it's better to differentiate data with [tags](../glossary/#tag) than with detailed measurement names.
...
* *Don't put more than one piece of information in one tag*
    ...
    Assume that each [tag set](../glossary/#tag-set) falls in the [measurement](../glossary/#measurement) `blueberries` and is associated with a [field](../glossary/#field) called `value`.
...
* *Don't use the same name for a field key and tag key*
    ...
    You won't be able to query the [tag key](../glossary/#tag-key) if the tag key is the same as a [field key](../glossary/#field-key) in your schema.
